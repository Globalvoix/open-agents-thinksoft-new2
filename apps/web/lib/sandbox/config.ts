 ...
 * Vercel API caps total sandbox timeout at 2,700,000 ms (45 min).
 * The SDK adds a 30-second beforeStop buffer, so the proactive timeout is 44.5 min. */
export const DEFAULT_SANDBOX_TIMEOUT_MS = 2_700_000 - 30_000; // 44.5 minutes
 ...
 * Set VERCEL_SANDBOX_BASE_SNAPSHOT_ID to a valid snapshot ID in your Vercel account
 * to pre-install tools (bun, jq, agent-browser, chromium, code-server).
 * If unset, sandboxes start from a clean Ubuntu environment.
 */
