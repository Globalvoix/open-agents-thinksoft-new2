...
/**
 * Strip null bytes (\u0000) from all string values in the diff response.
 * PostgreSQL JSON columns reject null bytes (code 22P05), and git diffs of
 * binary files (e.g. favicon.ico) can contain them.
 */
function stripNullBytes(response: DiffResponse): DiffResponse {
  ...
  return {
    ...
    files: response.files.map((file) => ({
      ...file,
      diff: clean(file.diff),
      ...(file.localDiff !== undefined && { localDiff: clean(file.localDiff) }),
    })),
  };
}
...
export async function computeAndCacheDiff(params: {
  ...
  if (baseRef === null) {
    ...
    updateSession(sessionId, {
      cachedDiff: stripNullBytes(response),
  ...
  // Strip null bytes from diff strings before storing — PostgreSQL JSON columns
  // reject \u0000 (code 22P05) which appears in diffs of binary files (e.g. favicon.ico).
  updateSession(sessionId, {
    cachedDiff: stripNullBytes(response),
    cachedDiffUpdatedAt: new Date(),
    linesAdded: response.summary.totalAdditions,
