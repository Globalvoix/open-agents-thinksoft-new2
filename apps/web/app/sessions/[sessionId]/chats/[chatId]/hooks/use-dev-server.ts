...
export function useDevServer({
  ...
  const handlePrimaryAction = useCallback(async () => {
    ...
    try {
      ...
      let lastError: string = "Failed to launch dev server";
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        ...
        try {
          response = await fetch(`/api/sessions/${sessionId}/dev-server`, {
            method: "POST",
          });
          body = await response.json().catch(() => null);
        } catch {
          lastError = "Network error connecting to dev server";
          continue;
        }
        ...
        lastError = getErrorMessage(body, "Failed to launch dev server");
