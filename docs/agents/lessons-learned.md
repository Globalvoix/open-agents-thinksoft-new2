...
- Task-subagent context only flows if the subagent's `callOptionsSchema` explicitly accepts that field; passing new context like `uiStudio` from `task.ts` is not enough on its own because unknown options are dropped before `prepareCall`.
